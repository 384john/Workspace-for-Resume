...
import com.nyulink.info.ActivityUsersInfo;
...
public class MyJson {
	...
	// 解析活动方法
	public List<VoiceInfo> getAshamedList(String value) {
		List<VoiceInfo> list = null;
		try {
			JSONArray jay = new JSONArray(value);
			list = new ArrayList<VoiceInfo>();
...
//				info.setQshare(job.getString("QSHARE"));
	...
	// 解析评论的方法
	public List<CommentInfo> getAhamedCommentsList(String value) {
	...
	public List<UserInfo> getNearUserList(String result) {
		...
		try {
			...
			for (int i = 0; i < jay.length(); i++) {
				...
				info.setUlat(job.getDouble("ULAT"));
				info.setUlng(job.getDouble("ULNG"));
				list.add(info);
			}
		} catch (JSONException e) {
		}
